...
* NOTE: `-d` is the attester's private key here, `-epk` is the epoch key of the receiver, `-i` is the proof index of the epoch key, `-pr` (optional) is the positive reputation given to the user, `-nr` (optional) is the negative reputation given to the user, `-gf` (optional) is the graffiti for the reputation given to the user, `-s` (optional) is the sign up flag to give to the user to indicate the attester authenticates the user's membership.
...
You can take a look at [`genUserStateFromContract`](https://github.com/appliedzkp/UniRep/blob/7e5cf425242134f73b6131778549b6039ea20a9b/core/utils.ts#L788) to better know how a user can fetch the events from the contract and build up the latest global state.
